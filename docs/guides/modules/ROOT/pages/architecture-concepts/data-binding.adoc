...
[NOTE]
====
. In this article `source` is used as any object in the code and `target` as any UI control (like xref:components::textfield.adoc[TextField]).
...
Part of the data-binding settings deals with how the data flow between the UI and the data object.
...
To raise this event, a built-in class is used, which provides this functionality — xref:guides::nativescript-core/observable.adoc[Observable]

include::partial$data-binding-1.adoc[]
...
See link:#parent-binding[Parent Binding] In the example from link:#binding-in-code[Binding in Code], the *bindingContext* can be set either on a xref:components::page.adoc[Page] instance or a xref:components::index.adoc#stacklayout[StackLayout] instance and the xref:components::textfield.adoc[TextField] will inherit it as a proper source for the binding of its "text" property.
...
For example, xref:components::listview.adoc[ListView] creates its child items based on an itemТemplate, which describes what the ListView element will look like.
When this element is added to the visual tree, it gets for binding context and element from a ListView items array (with the corresponding index).
...
include::partial$data-binding-2.adoc[]

[NOTE]
====
If the value of the `items` property of the `ListView` is an array of plain elements(numbers, string, dates) as in the preceding example, you use the `$value` variable to access the current item of the array.
...
Custom expressions could help in cases when certain logic should be applied to the UI, while keeping the underlying business data and logic clear.
...
Since nestedProp is not a property of the bindingContext in _item.nestedProp,_ then there will be no propertyChange listener attached and changes to _nestedProp_ will not be populated to UI.
...
NativeScript supports different kinds of expressions, including:
...
|`property access`
|`obj1.obj2.prop1`
|Resulting in the value of the `prop1` property of the object `obj2`. Expressions in binding are based on polymer expressions, which supports re-evaluation of expression when any value within the dot (`.`) chain is changed. NativeScript uses expressions only in the context of bindings (for now), so a binding expression will be re-evaluated only when the binding source property is changed (due to performance considerations).The expression part will not observe and, therefore, will not initiate re-evaluation.
|array access
|`arrayVar[indexVar]`
...
A Converter object should have one or two functions (`toView()` and `toModel()`) executed every time when data should be converted.
...
The best way to get a date from a user is to use a xref:components::datepicker.adoc[DatePicker].
====
...
However, this is not the case when xref:components::listview.adoc[ListView] items should be bound.
The problem comes from the fact that the bindingContext of a ListView is the individual data items in the array, and to apply a converter, the converter and its parameters should be added to each data item, which will result in multiple converter instances.
Tackling this problem with NativeScript is fairly simple.
the binding infrastructure searches for the application level resources to find a proper converter and parameters.
