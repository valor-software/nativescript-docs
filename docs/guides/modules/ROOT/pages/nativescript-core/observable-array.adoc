...
The `ObservableArray` class expands the Javascript `Array` class by providing a capability of detecting and responding to changes of a collection of objects.
...
Once you have the app, scan the QR with your phone Camera and the app resulting from the code in the IDE will appear in the `Nativescript Preview` app.
...
If omitted, the elements are sorted into ascending, ASCII character order.
