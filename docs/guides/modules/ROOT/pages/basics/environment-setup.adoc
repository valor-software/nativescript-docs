...
image::./assets/environment-setup/new_user_variable_dialog.png[New User Variable (ANDROID_HOME)]
...
image::/assets/environment-setup/xcode_command_line_tools.png[XCode Preferences, Locations]
...
:warning: *Note*: Python 3 is fully supported by the NativeScript components that rely on it, however changing our scripts to use the `python3` executable name by default is a minor breaking change we're aiming to introduce in NativeScript 8.3.
...
:::tip Note You may see a Deprecation warning when installing *six*, feel free to ignore it for now &mdash;
...
_*More details for those curious:* The NativeScript CLI relies on 3rd party packages that may have been deprecated over the past years.
...
:::warning Note We have gone through these steps on *Ubuntu 20.04* and noted commands we've used, however depending on your Linux distribution, the commands may be different.
...
:::tip Note Depending on how you installed *Node*, you may get an `EACCESS: permission denied` error when trying to install a global package.
...
In the same screen verify that *Objective-C* is selected as your language.
...
. Inside the *_Info.plist_* of the Watch Extension replace the of *_`WKAppBundleIdentifier`_* with *_`$(WK_APP_BUNDLE_IDENTIFIER)`_*.
