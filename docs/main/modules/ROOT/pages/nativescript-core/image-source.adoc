...
+ `options` : Optional parameter, Only used for `Android`, `options.filter` is a boolean which determines whether or not bilinear filtering should be used when scaling the bitmap.
