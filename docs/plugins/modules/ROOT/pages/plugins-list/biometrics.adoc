...
Since iOS9 it's possible to check whether or not the list of enrolled fingerprints changed since the last time you checked it.
...
The idea being that the key will not be accessable unless the user has successfully authenticated.
...
Allowing the user to fallback on lock screen credentials ( pin etc.
) disables cryptography.
