...
These snapshots provide the ability to view the data, view query metadata (such as whether the data was from local cache), whether the document exists or not and more.
...
The https://firebase.google.com/docs/firestore/manage-data/structure-data[Firebase Documentation] provides some great examples on the best practices to structuring your data.
...
If you simply want to write multiple documents without using the document's current state, a batch write would be more appropriate.
...
This functionality is enabled by default, however it can be disabled if you need it to be disabled (e.g.
